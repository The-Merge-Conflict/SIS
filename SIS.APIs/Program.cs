...
builder.Services.AddHttpClient<IActivityLogClient, ActivityLogClient>(client =>
{
    var baseUrl = builder.Configuration["LoggingService:BaseUrl"] ?? "http://localhost:5110";
...
builder.Services.AddOpenApi(options =>
{
    _ = options.AddDocumentTransformer((document, context, cancellationToken) =>
    {
        ...
        // Use the interface type for using FluentValidation;
using FluentValidation.AspNetCore;
using Microsoft.AspNetCore.Authentication.JwtBearer;
using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;
...
// app.UseHttpsRedirection(); // Commented out to fix the warning
...
app.Run();
the variable to ensure compatibility
        IOpenApiSecurityScheme jwtScheme = new OpenApiSecurityScheme
        {
            Type = SecuritySchemeType.Http,
            Scheme = "bearer",
